...
def hhmmss_to_hms(hhmmss):
    """
    hhmmss_to_hms(hhmmss):
       Convert hhmmss.s float into hours, minutes and sec
    """
    if hhmmss < 0.0: sign = -1
    else: sign = 1
    hms = abs(hhmmss)
    hour = sign * int(hms/10000.)
    min = int((hms - abs(hour) * 10000) / 100.)
    sec = hms - abs(hour) * 10000 - min * 100
